...
///
/// 一自定义的NestedScrollView对象
///
/// Flutter系统自带的NestedScrollView如果使用PageView为body且PageView的子页面是ListView时，
/// 当滑动一个页面的ListView其他不可见的ListView也会同步滑动。
///
/// 当下拉时，当PageView中任意一个ListView滑动顶部时都会拉出Header，这是不正确的，我们应该只判断当前页面的ListView是否滑动到了顶部。
///
/// 这个NestedScrollView解决了这个问题。
///
///
class GNestedScrollView extends StatefulWidget {
  ...
  List<Widget> _buildSlivers(BuildContext context, ScrollController innerController, bool bodyIsScrolled) {
    return <Widget>[
      ...headerSliverBuilder(context, bodyIsScrolled), // 构建头部

      SliverFillRemaining( // 构建body
        child: PrimaryScrollController(
          controller:  innerController, // 内部的控制器
          child: body,
        ),
      ),
    ];
  ...
  @override
  GNestedScrollViewState createState() => GNestedScrollViewState();
}

class GNestedScrollViewState extends State<GNestedScrollView> {
  ...
  void initState() {
    ...
    // 如果body是PageView要做特殊的处理
    if (widget.body is PageView) {

      // 监听PageView的滑动，
      final PageView pageView = widget.body as PageView;
      ...
      pageController.addListener(() {
        ...
        if (currentPage != currentPageSelectIndex) {
          currentPageSelectIndex = currentPage;
          print("onPageChagne:$currentPageSelectIndex");

          _coordinator?.currentPageIndex = currentPageSelectIndex;
        }
      });
    }
  }
  ...
  @override
  void didUpdateWidget(GNestedScrollView oldWidget) {
  ...
  void _handleHasScrolledBodyChanged() {
    ...
    if (_lastHasScrolledBody != newHasScrolledBody) {
      // print("hasScrolledBody:$newHasScrolledBody");
  ...
  Widget build(BuildContext context) {
    ...
    print("NestedScrollViewState build");
      ...
      child: Builder(
        builder: (BuildContext context) {
          ...
          return _NestedScrollViewCustomScrollView(
            ...
            controller: _coordinator!._outerController, // 外部的控制器
...
class _InheritedNestedScrollView extends InheritedWidget {
  ...
  final GNestedScrollViewState state;
...
class _NestedScrollCoordinator implements ScrollActivityDelegate, ScrollHoldController {
  ...
  _NestedScrollCoordinator(
    ...
    print("_NestedScrollCoordinator:$_parent");
  ...
  final GNestedScrollViewState _state;
  ...
  bool get hasScrolledBody {
    ...
    // print("hasScrolledBody _innerPositions:$_innerPositions");
...
class _NestedScrollController extends ScrollController {
  ...
  _NestedScrollController(
    ...
    // coordinator = _NestedScrollCoordinator(_state, _parent, () { }, _floatHeaderSlivers)
  }
...
/// and an [NestedScrollViewViewport], to shift overlap in a [GNestedScrollView].
///
...
/// See also:
///
///  * [GNestedScrollView], which uses a [NestedScrollViewViewport] and a
...
/// See also:
///
///  * [GNestedScrollView], whose documentation has sample code showing how to
///    use this widget.
...
/// See also:
///
///  * [GNestedScrollView], which uses a [SliverOverlapAbsorber] to align its
...
/// The [Viewport] variant used by [GNestedScrollView].
///
...
class NestedScrollViewViewport extends Viewport {
  ...
  void updateRenderObject(BuildContext context, RenderNestedScrollViewViewport renderObject) {
    print("NestedScrollViewViewport updateRenderObject");
...
/// The [RenderViewport] variant used by [GNestedScrollView].
///
/// This viewport takes a [SliverOverlapAbsorberHandle] and notifies it any time
/// the viewport needs to recompute its layout (e.g. when it is scrolled).
